...
public class Main {
    ...
    public static void main(String[] args) throws SQLException {
        // اتصال به دیتابیس
        connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/citywarstokyo", "root", "1384483100");
        loadUsersFromDatabase(connection);

        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            saveUsersToDatabase(connection);
        }));
...
////////////////////////////////////////////\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
        ...
        while (run) {
            ...
            if (BackMenuMainMatcher.matches()) {
                currentMenu[0]=menu.Main_Menu;
                System.out.println("you have entered to the Main Menu !");
            }
/////////////////////////////////////////////ModeMenu\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
//        }
    ...
    public static void HistoryGame(Scanner input,menu[]currentMenu){
        int numHistory=1;
        for (int i = 0; i < loginedUser.historyGamePlayer.size(); i++) {
            if (i < 10) {
    ...
    private static void loadUsersFromDatabase(Connection connection) {
        try {
            ...
            try (PreparedStatement stmt = connection.prepareStatement(query);
                ...
                while (rs.next()) {
                    ...
                    user.setXPlevelup(rs.getInt("XPlevelup"));
                    user.coin = rs.getDouble("coin");
                    user.setCharecter(rs.getString("charecter"));

                    // Load special cards
                    user.specialCardsPlayer = (ArrayList<SpecialCard>) SpecialCard.loadFromDatabase(connection, user.getId());

                    // Load cards
                    user.CardsPlayer = (ArrayList<Card>) Card.loadFromDatabase(connection, user.getId());

                    // Load game history
                    user.historyGamePlayer = (ArrayList<HistoryGame>) HistoryGame.loadFromDatabase(connection, user.getId());
    ...
    private static void saveUsersToDatabase(Connection connection) {
        try {
            // ابتدا تمامی کاربران و داده‌های مرتبط را از دیتابیس پاک کنید
            try (Statement stmt = connection.createStatement()) {
                stmt.executeUpdate("DELETE FROM users");
                stmt.executeUpdate("DELETE FROM special_cards");
                stmt.executeUpdate("DELETE FROM cards");
                stmt.executeUpdate("DELETE FROM history_games");
            }

            // سپس تمامی کاربران و داده‌های مرتبط را به دیتابیس اضافه کنید
            for (User user : users) {
